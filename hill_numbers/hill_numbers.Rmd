...
```{r include=FALSE}
setwd('~/Documents/GitHub/Plasticity_Exp_Repo/hill_numbers/')
library(tidyverse)
library(vegan)
library(nlme)
library(hilldiv)
...
```{r include=FALSE}
Crocidura_products <- read.csv('../damma/outputsR/distilled_table_functions_CR.csv', sep=',', row.names = 1) %>%
...
#####Apodemus 
Apodemus_products <- read.csv('../damma/outputsR/distilled_table_functions_AS.csv', sep=',', row.names = 1) %>%
